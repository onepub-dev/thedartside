...
Its 2am and I can't sleep, the rain is pouring down and the wind is howling. Like a lone fisherman searching for a safe harbour here I stand in the middle of the night looking for a way to safely store my App's data.
...
So The Dart Side is going to do a brief tour of some of the alternatives and then we are going to explore one of the more traditional methods and what for most developer should probably the cornerstone of how they store data.

Because its 2am let's have some fun and make some stuff up.
...
The down side to local storage, is just that, the storage is local. If your user's loses their phone or has multiple devices then can't access the data stored on their device.
...
The more traditional approach is to use an SQL store such as SQLlite.  SQLite has a number of advantages, its a traditional SQL database that we all know, love and hate. SQLite is available through packages like [sqflite](https://pub.dev/packages/sqflite) which make it easy to integrate with Flutter.

Having a complete set of SQL commands at your disposal gives you a lot of power over your data. SQLite also support indexes which can be critical for performance when you need to access data via complex queries or in large tables.
...
Now imagine if you app crashes after step 4. Without a transaction, you have just lost real money. The 'from' account has had value removed, but that value hasn't been transferred into the 'to' account.
...
If you have a large no of related entities, SQLite is a better choice than Hive.
...
So know we get to the meat of what The Dart Side is about.  Using Dart on the server.
...
A Client/Server model allows you to store your data in the cloud but also provide the ability to Transform your data off device providing a complete TIDEL implementation without the processing limitations of you typical mobile phone.
...
The Web Server is typically an Apache or Nginx Web Server which sites in front of you Application Server.
...
{% hint style="info" %}
The Web Server will likely maintain a pool of already established connections to your Application Server to reduce the overheads of establish a new connection for each request.
{% endhint %}

The Application Server accepts HTTP requests from you Flutter App \(via the Web Server\).
...
There are a number of ways you can format your request when you send data to you Application Server.
